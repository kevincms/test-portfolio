...
date: 2026-08-26
lastmod: 2026-09-11
summary: "Project Context의 효과를 정량적으로 살펴보기 위해 PDF 처리 작업 78세션을 실행했다. 기록 방식에 따른 입력량·절차 관리·정보 보존을 비교하고, 평가의 한계와 후속 과제를 정리했다."
draft: false
tags:
  - Side Project
  - AI Engineering
...
highlights:
  ...
  - "역할 분리 조건의 평균 입력량 19.5% 감소 관찰, 품질 동등성은 미확인"
  - "절차 재사용 지표의 한계를 정리하고 밈 언어 기반 후속 평가 착수"
---

<style>
main:has(#pcb-summary) h1{word-break:keep-all;overflow-wrap:break-word}
...
.pcb-table table{width:100%;table-layout:fixed;font-size:.85rem;line-height:1.65}
.pcb-table :where(th,td){vertical-align:top;word-break:keep-all;overflow-wrap:anywhere}
.pcb-table th:first-child{width:29%}
.pcb-table code{white-space:normal;overflow-wrap:anywhere;font-size:.76rem}
...
역할 분리 조건의 세션당 평균 입력 토큰은 **19.5% 낮았다**. 절차를 일찍 기록하는 모습도 관찰했지만, 출력 품질의 동등성과 절차 재사용의 우위는 확인하지 못했다. 이 글은 완료된 PDF 탐색 실험의 설계와 결과, 그 결과를 해석할 수 있는 범위를 정리한 기록이다.
...
<div class="pcb-metric"><span>평균 입력 토큰</span><strong>−19.5%</strong><small>C2 대비 C3의 관찰값</small></div>
<div class="pcb-metric"><span>품질 동등성</span><strong>미확인</strong><small>정보 보존 점수는 C3 ≤ C2</small></div>
</div>
...
<a href="#pcb-design"><span class="pcb-roadmap-number">02</span><strong>설계</strong><small>PDF · 두 기록 방식</small></a>
...
실험에서는 기존 조건 이름인 **C2와 C3**를 사용했다. C2는 `AGENTS.md` 한 파일에 기록을 모으고, C3는 `AGENTS.md`와 역할별 상태·지식 문서로 나누어 보존하는 조건이다.

비교의 초점은 **작업 중 남긴 기록을 어떤 구조로 관리하는가**였다. 이번 비교에는 기록을 남기지 않는 조건이 포함되지 않았다.
...
<div class="pcb-panel"><div class="pcb-panel-title"><strong>역할별로 분리</strong><small>C3</small></div><ul class="pcb-records"><li><code>AGENTS.md</code><span>역할별 문서와 함께 사용하는 규칙 파일</span></li><li><strong>상태·지식 문서</strong><span>기록을 역할에 맞는 문서로 나누어 보존</span></li></ul></div>
</div>
...
| 정보 보존 | 표·수식·도표 등의 fidelity recall | 원문 정보가 결과물에 얼마나 남았는가 |

</div>

정보 보존 평가는 정답 정보와 결과물을 비교했다. 표는 셀과 행·열 관계, 수식은 구조와 기호, 도표는 유형에 따라 코드 줄·노드와 연결·기재된 사실 등을 살폈다. 이 점수는 결과물의 모든 품질을 대표하지는 않지만, 입력량이 줄 때 정보 보존까지 함께 확인하기 위한 기준이었다.
...
<p class="pcb-chart-note"><strong>C3 평균 입력량 −19.5%.</strong> 같은 실험에서 품질 동등성과 재사용 우위는 확인되지 않았다.</p>
...
세부적으로 C3의 단계 수는 6.0%, 단계당 입력량은 14.4% 낮았다. 입력량 차이가 단계 수의 감소만으로 설명되지는 않았다. 다만 단계당 입력에는 문서뿐 아니라 도구 실행 결과와 누적 대화도 포함되므로, 이 수치를 문서를 읽는 양의 감소율로 그대로 볼 수는 없다.
...
| 최종 기록의 용량·파일 수 | 상대적으로 작음 | 3회 모두 C2보다 큼 |
| 노트 쓰기 호출 | 상대적으로 적음 | 3회 모두 C2보다 많음 |

</div>

C3는 모든 반복에서 첫 세션부터 절차를 기록했다. C2도 세 번 모두 절차를 형성했으며, 그중 한 번은 7번째 세션에 처음 기록했다. 따라서 차이는 절차 형성의 성공 여부보다 **형성 시점과 관리 빈도**에 있었다.

또한 C3에서는 더 많은 기록을 보존하면서 평균 입력량은 낮아지는 패턴이 나타났다. 저장할 정보와 현재 세션에 읽힐 정보를 분리할 가능성을 보여주는 관찰이다. 한편 노트 쓰기 호출도 늘었으므로, 기록을 유지하는 작업량까지 함께 살펴야 한다.

### 3.3 출력 품질의 동등성과 재사용 우위는 미확인이었다

원문 정보 보존 점수인 **fidelity recall은 세 번 모두 C3가 C2 이하**였다. 입력 토큰이 낮았다는 결과만으로 같은 품질을 더 적은 비용으로 얻었다고 결론 내릴 수는 없었다.
...
특히 이번 재사용 지표는 두 조건의 차이를 안정적으로 판별하지 못했다. 너무 엄격하거나 느슨한 기준의 한계와 행동 기반 판정의 변동을 함께 확인했다. 더 분명한 비교를 하려면 재사용의 정의와 채점 기준부터 보완할 필요가 있었다.

### 4.2 입력량 차이의 원인과 품질 기준이 남았다

기록을 역할별로 나누는 방식은 낮은 입력량과 함께 관찰됐다. 그러나 단계 수, 도구 사용 경로, 노트를 읽는 행동의 차이도 영향을 줄 수 있어 역할 분리 자체의 효과를 따로 입증하지는 못했다.
...
이 부족함을 바탕으로 **밈 언어 기반 평가를 새로 시작했다**. 현재 진행 중이며, 이 페이지의 수치는 먼저 완료된 PDF 실험에 한정한다.
...
이 글은 내부 실험 보고서 「**PDF Arc Probe v0**」(2026-08-26)와 LLM Wiki에 정리한 검토 내용을 바탕으로 작성했다. 보고서에 포함된 집계 수치는 [요약 데이터(YAML)](results-summary.yaml)로 함께 남겼다. 요약 데이터에는 원시 세션 로그나 개별 실행 결과는 포함하지 않았다.
