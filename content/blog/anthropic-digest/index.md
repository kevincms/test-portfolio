...
분기별로 각 테마가 언급된 글의 비율(%)입니다. 시간이 흐를수록 **Agent**와 **Tool use/MCP**가 오르고, **Interpretability**는 50%대에서 한 자릿수로 내려앉습니다. **Safety/Align**은 전 기간 가장 높지만 후반부(2025 말~)로 갈수록 점유율을 내주고, **Eval/Benchmark**는 25Q1(55%)에 피크를 찍습니다. 범례에서 테마 이름을 클릭하면 개별 추세선을 켜고 끌 수 있고, 차트에 마우스를 올리면 확대·팬·PNG 저장 도구가 나타납니다.
...
- **2026 — 에이전트 엔지니어링 + 도구 표준화**: 에이전트가 연구 주제에서 **엔지니어링 실무**로 이동(`Context engineering`, `Multi-agent system`, `Harness design`, `Agent Skills`). **Tool use/MCP**가 26Q2에 40%로 정점 — MCP 표준화(재단 기증)·도구 호출이 에이전트의 핵심 인터페이스로 자리 잡음.

곁가지로 읽히는 두 축:

- **Safety의 의미 변화** — "모델 내부 이해(interpretability)" → "실무 안전장치·악용 탐지·사이버 방어"로 무게중심 이동. 점유율은 내려갔지만 사라진 게 아니라 *형태*가 바뀐 것.
- **Agent → Tool/MCP로 이어지는 흐름** — 2025 에이전트 담론이 2026 들어 **도구 사용·MCP 표준화**라는 구체적 엔지니어링으로 수렴(Tool use/MCP 15→40%).
...
- **2025-07-23** · [Claude's extended thinking](https://www.anthropic.com/research/visible-extended-thinking)

    Anthropic은 복잡한 문제에 대해 더 깊이 사고하고 그 과정을 투명하게 공개할 수 있는 확장된 사고 모드를 탑재한 새로운 인공지능 모델 Claude 3.7 Sonnet을 출시했다는 결론을 발표했습니다. 이 모델은 자율적인 컴퓨터 사용이나 게임 플레이와 같은 연속적이고 개방형인 agent 작업 수행 능력이 크게 향상되었으며 유해한 사고 과정을 암호화하고 prompt 주입 공격을 방어하는 새로운 안전장치를 통해 기존의 2단계 인공지능 안전 기준을 충족했습니다. 스스로 사고 시간을 조절하고 투명성을 높인 이러한 혁신은 향후 개발자들이 더욱 강력하고 정교한 인공지능 agent를 구축하는 데 기여할 뿐만 아니라 모델의 신뢰성 확보와 안전성 강화를 위한 새로운 연구의 중요한 이정표가 될 것입니다.
